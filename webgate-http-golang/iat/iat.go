...
//测试唤醒词引擎
func iatClient(serverUrl string, reqData []byte, client *fasthttp.Client) {
...
func main() {
	//定义命令行参数
	flag.Usage = Usage
	url := flag.String("url", "https://rest-api-vanke.xfyun.cn/v2/iat, "connect server url")
	res := flag.String("res", "dddd.pcm", "res path")
	flag.Parse()
	audioFile1 := *res
